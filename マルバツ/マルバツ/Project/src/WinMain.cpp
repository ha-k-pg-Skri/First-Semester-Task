...
{
     if (Stone[y] [x]== true) //盤面の x, y の位置に石が置けるならtrue,置けないならfalseを返す処理
	{
		return false;
	}
...
int CheckWinner()
{
	...
	int white_RevereseSlash_counter = 0;//斜め
	int black_RevereseSlash_counter = 0;
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (map[i][i] == STONE_WHITE)white_RevereseSlash_counter++;
		if (map[i][i] == STONE_BLACK)black_RevereseSlash_counter++;

		if (white_horizontal_counter == 3 || white_vertical_counter == 3 || white_Slash_counter == 3 || white_RevereseSlash_counter == 3)
			return WINNER_WHITE;

		if (black_horizontal_counter == 3 || black_vertical_counter == 3 || black_Slash_counter == 3 || black_RevereseSlash_counter == 3)
			return WINNER_BLACK;
	}
    // 縦、横、斜めが同じ石かどうかを調べる
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3 ;j++)
		{
			...
			{
				return WINNER_NON;// 上記のいずれかでも無かったらWINNER_NONを返す
			}
