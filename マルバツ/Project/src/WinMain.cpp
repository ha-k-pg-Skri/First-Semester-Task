...
// 横:STAGE_WIDTH、
//縦:STAGE_HEIGHTのint型２次元配列 map を宣言
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// 各種初期化処理
	// ----------------------------------------------------
	void InputInit();			// 入力処理初期化関数の呼び出し
	void DrawInit();				// 描画処理初期化関数の呼び出し
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		if(winner== WINNER_NON)
		{
			...
			else if(IsPushKey(KEY_INPUT_DOWN))
			{
				pos_y++;	//pos_yの値を 1 増やす
			}
			else if(IsPushKey(KEY_INPUT_LEFT))
			{
				pos_x--; // pos_xの値を 1 減らす
			}
			else if(IsPushKey(KEY_INPUT_RIGHT))
			{
				pos_x++; //pos_xの値を 1 増やす
			}
			// 決定(=エンターキー)が押された時の処理
			else if(IsPushKey(KEY_INPUT_RETURN))
			{
				...
				if (IsPutStone(pos_x,pos_y)==true)
				{
					...
					// 以下の処理を実装する
					map[pos_x][pos_y] = turn;  // 選択されている座標と対応するmap配列の要素へturnの値を代入
		...
		void DrawStone(int x, int y, int stone);//　２重for文を使って盤面の石を描画する

		for (int i = 0; 0 < pos_x; i++)
		{
			
			for (int j = 0; 0 < pos_y; j++)
			{
...
bool IsPutStone( int x, int y )
{
	...
	return false;//　盤面の x, y の位置に石が置けるならtrue,置けないならfalseを返す処理
}
...
int CheckWinner()
{
	...
	int white_Slash_counter = 0;
	int black_Slash_counter = 0;

	for (int i = 0; i < 3; i++)
	{
		int white_horizontal_counter = 0;
		int black_horizontal_counter = 0;

		int white_vertical_counter = 0;
		...
		for (int j = 0; j < 3; j++)
		{
			...
			if (map[i][j] == STONE_WHITE)white_vertical_counter++;
			if (map[i][j] == STONE_BLACK)black_vertical_counter++;
		}
